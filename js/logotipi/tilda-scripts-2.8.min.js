...
return!1};VideoLoadProcessor.prototype.unergisterVideo=function(video){if(!(video instanceof HTMLVideoElement)){throw new Error('Wrong tag passed into unregisterNewVideo')}
...
return null};VideoLoadProcessor.prototype.getVideoBoundingRect=function(video,isNeedParent){if(typeof isNeedParent=='undefined'){isNeedParent=!0}
var parent=null;if(isNeedParent){parent=$(video).parents('.r')[0];if(!parent){parent=video}}else{parent=video}
return parent.getBoundingClientRect()};window.videoLoadProcessor=new VideoLoadProcessor()})(jQuery);(function($){function SequenceController(){this.setScrollCb();this.itemHeight=screen.availHeight;var itemTransitionItemRelation=0.25;this.itemTransitionTop=this.itemHeight*itemTransitionItemRelation;this.activeItemIndex=null;this.windowHeight=document.documentElement.clientHeight||window.innerHeight||screen.availHeight;this.topOffsetShift=-150;$(window).resize(jQuery.proxy(this.recalculateAllSequencesOffsets,this));this._resizeInterval=setInterval(jQuery.proxy(this.scrollCb,this),500)}
...
this._resizeTimeout=setTimeout(jQuery.proxy(function(){this.scrollCb();this._resizeInterval=setInterval(jQuery.proxy(this.scrollCb,this),500)},this),10)};SequenceController.prototype.registerNewBlock=function(node){if(!(node instanceof HTMLElement)){throw new Error('Wrong node type in registerNewBlock')}
...
if(window.isMobile&&(videowebm!=''||videomp4!=''||youtubeid!='')){el.css('background-image',"url('"+backgroundurl+"')")}
setTimeout(function(){cover_fixcontentheight(id);cover_fixBackgroundFixedStyles(id)},500);cover_fixBackgroundFixedNode(id);var clogo=$('#rec'+id).find('img[data-hook-clogo]');if(clogo.length){clogo.load(function(){setTimeout(function(){cover_fixcontentheight(id);cover_fixBackgroundFixedStyles(id)},500)})}
if(window.isMobile){$(window).on('orientationchange',function(){cover_fixcontentheight(id);cover_fixBackgroundFixedStyles(id)})}
if(videomp4!==''||videowebm!==''||youtubeid!==''){if(window.isMobile==!1){if(youtubeid==''&&(videomp4!=''||videowebm!='')){el.css('background-color','#000000');el.css('background-image',"url('https://tilda.ws/img/spinner-white.gif')");el.css('background-size','auto');el.attr('data-content-cover-bg','');var loop=!1;var muted=!0;if(noloop!=''){loop=!1}else{loop=!0}
...
if(parallax=='dynamic'){if(window.isMobile==!1)el.parallax('50%',0.2,!0)}
...
var coverarrow=$('#rec'+id).find('.t-cover__arrow-wrapper');if(coverarrow.length>0){coverarrow.click(function(){var recheight=$('#rec'+id).height();if(recheight>0){$('html, body').animate({scrollTop:$('#rec'+id).offset().top+recheight},500)}})}})};function cover_fixcontentheight(id){var el=$('#rec'+id);var hcover=el.find('.t-cover').height();var hcontent=el.find('div[data-hook-content]').outerHeight();if(hcontent>300&&hcover<hcontent+40){var hcontent=hcontent+120;if(hcontent>1000){hcontent+=100}
console.log('auto correct cover height: '+hcontent);el.find('.t-cover').height(hcontent);el.find('.t-cover__filter').height(hcontent);el.find('.t-cover__carrier').height(hcontent);el.find('.t-cover__wrapper').height(hcontent);if(window.isMobile==!1){setTimeout(function(){var divvideo=el.find('.t-cover__carrier');if(divvideo.find('iframe').length>0){console.log('correct video from cover_fixcontentheight');setWidthHeightYoutubeVideo(divvideo,hcontent+'px')}
...
function cover_checkIsFixForBackgroundNeeded(id){var rec=document.body.querySelector('#rec'+id);if(!rec){return}
var coverCarrier=rec.querySelector('.t-cover__carrier');var youtubeUrl=$(coverCarrier).attr('data-content-video-url-youtube');var mp4Url=$(coverCarrier).attr('data-content-video-url-mp4');var webmUrl=$(coverCarrier).attr('data-content-video-url-webm');var is_safari=/^((?!chrome|android).)*safari/i.test(navigator.userAgent);var isMobile=window.isMobile||(/Macintosh/.test(navigator.userAgent)&&'ontouchend' in document);if(!isMobile&&is_safari&&!youtubeUrl&&!mp4Url&&!webmUrl&&coverCarrier&&coverCarrier.dataset.contentCoverParallax=='fixed'&&!window['cover'+id+'fixbackgroundstyles']&&(rec.dataset.recordType==18||rec.dataset.recordType==14||rec.dataset.recordType==46||rec.dataset.recordType==114||rec.dataset.recordType==167||rec.dataset.recordType==168||rec.dataset.recordType==192||rec.dataset.recordType==207||rec.dataset.recordType==213||rec.dataset.recordType==204||rec.dataset.recordType==205||rec.dataset.recordType==206||rec.dataset.recordType==229||rec.dataset.recordType==244||rec.dataset.recordType==338||rec.dataset.recordType==274||rec.dataset.recordType==448||rec.dataset.recordType==288||rec.dataset.recordType==716||rec.dataset.recordType==712||rec.dataset.recordType==411||rec.dataset.recordType==415||rec.dataset.recordType==476||rec.dataset.recordType==160||rec.dataset.recordType==237||rec.dataset.recordType==238)){return!0}else{return!1}}
function cover_fixBackgroundFixedNode(id){var rec=document.body.querySelector('#rec'+id);if(cover_checkIsFixForBackgroundNeeded(id)){console.log('new fix node background-position: fixed');if(!window.cover_fixBackgroundStyles){var css='.t-cover__container {position: relative;}.t-cover__container .t-cover {clip: rect(0, auto, auto, 0);position: absolute;top: 0;left: 0;width: 100%;height: 100% !important;}.t-cover__container .t-cover .t-cover__carrier {position: fixed;display: block;top: 0;left: 0;width: 100%;height: 100%!important;background-size: cover;background-position: center center;transform: translateZ(0);will-change: transform;}',head=document.head||document.getElementsByTagName('head')[0],style=document.createElement('style');head.appendChild(style);style.type='text/css';if(style.styleSheet){style.styleSheet.cssText=css}else{style.appendChild(document.createTextNode(css))}
window.cover_fixBackgroundStyles=!0}
var newWrapper=document.createElement('div');newWrapper.classList.add('t-cover__container');rec.append(newWrapper);var cover=rec.querySelector('.t-cover');var coverHeight=cover.style.height;newWrapper.style.height=coverHeight;newWrapper.append(cover);window['cover'+id+'fixbackgroundnodes']=!0}}
function cover_fixBackgroundFixedStyles(id){var rec=document.body.querySelector('#rec'+id);if(cover_checkIsFixForBackgroundNeeded(id)){console.log('new fix style background-position: fixed');var newWrapper=rec.querySelector('.t-cover__container');var cover=rec.querySelector('.t-cover');var coverHeight=cover.style.height;cover.style.height=0;newWrapper.style.height=coverHeight;window['cover'+id+'fixbackgroundstyles']=!0}else{return}}
...
setWidthHeightYoutubeVideo(el,height)};def.then(defFunc)};function load_youtube_api(){if(window.loadytapi_flag!=='yes'){window.loadytapi_flag='yes';var tag=document.createElement('script');tag.src='https://www.youtube.com/iframe_api';var firstScriptTag=document.getElementsByTagName('script')[0];firstScriptTag.parentNode.insertBefore(tag,firstScriptTag)}}
window.onYouTubeIframeAPIReady=function(){def.resolve()};function setWidthHeightYoutubeVideo(el,height){console.log('setWidthHeightYoutubeVideo:'+height);var iframe=el.find('iframe');var nocover=el.attr('data-content-video-nocover');var noadcut=el.attr('data-content-video-noadcut-youtube');var customratio=el.attr('data-content-video-ratio');var video_ratio=0.5625;if(customratio>0)video_ratio=parseFloat(customratio)*1;if(nocover!='yes'){if(!height){height='100vh'}
...
evtCallbacks.push({callback:callback,config:typeof config=='object'?config:this.defaultConfig,})};Observer.prototype._getEventCallbacks=function(name){return this.callbacks[name]};Observer.prototype.removeEventListener=function(name,callback){var cbs=this._getEventCallbacks(name);if(!cbs){return!1}
for(var i=0,l=cbs.length,cbObj;i<l;i++){cbObj=cbs[i];if(callback===cbObj.callback){cbs.splice(i,1);return!0}}
return!1};Observer.prototype.emitEvent=function(name,data){var cbs=[];extend(cbs,this._getEventCallbacks(name));for(var i=0,l=cbs.length,cbObj,cb,config;i<l;i++){cbObj=cbs[i];cb=cbObj.callback;config=cbObj.config;if(config.context){cb.call(config.context,data)}else{cb(data)}
if(config.single){this.removeEventListener(name,cb)}}};window.observer=new Observer()})(jQuery);(function($){$(document).ready(function(){if(window.isMobile==!1&&$('#allrecords').attr('data-blocks-animationoff')!=='yes'&&window.isSearchBot==!1){$('.r').each(function(i){if($(this).attr('style')&&$(this).attr('style').indexOf('background-color')!==-1){$(this).attr('data-animationappear','off')}});var tiles=$('.r').not('[data-animationappear=off], [data-screen-min], [data-screen-max]'),wnd=$(window);tiles.each(function(i){a=$(this).offset().top;b=wnd.scrollTop()+wnd.height()+300;if(a>1000&&a>b){$(this).addClass('r_hidden')}else{$(this).addClass('r_showed')}
...
if($('html').css('display')==='none'){$('html').css('display','block')}
...
setWidthHeightHTMLVideo(video,options.height);return video.find('video')[0]};$.fn.videoBG.setFullscreen=function($el){var windowWidth=$(window).width(),windowHeight=$(window).height();$el.css('min-height',0).css('min-width',0);$el.parent().width(windowWidth).height(windowHeight);if(windowWidth/windowHeight>$el.aspectRatio){$el.width(windowWidth).height('auto');var height=$el.height();var shift=(height-windowHeight)/2;if(shift<0){shift=0}
...
$video.bind('canplay',function(){if(options.autoplay){v.play()}});if($.fn.videoBG.supportsVideo()){if($.fn.videoBG.supportType('webm')&&options.webm!=''){$video.attr('src',options.webm)}else{if($.fn.videoBG.supportType('mp4')&&options.mp4!=''){$video.attr('src',options.mp4)}else{$video.attr('src',options.ogv)}}}
...
return $div};$.fn.videoBG.supportsVideo=function(){return document.createElement('video').canPlayType};$.fn.videoBG.supportType=function(str){if(!$.fn.videoBG.supportsVideo()){return!1}
...
var virtProduct={name:productObj.name,price:productObj.price,variant:variant_str,quantity:1,};if(productObj.id&&productObj.id>0){id=productObj.id;virtProduct.id=productObj.id}
if(productObj.uid&&productObj.uid>0){uid=productObj.uid;virtProduct.uid=productObj.uid}
...
if(priceListID>0&&eventParams&&eventParams.currency_code){VK.Retargeting.Event('purchase');VK.Retargeting.ProductEvent(priceListID,eventName,eventParams)}else{VK.Retargeting.Event(eventName)}}else{VK.Retargeting.Event(virtPage)}}catch(e){}}
...
if(TILDAPAGE_UTM.length>0){var date=new Date();date.setDate(date.getDate()+30);document.cookie='TILDAUTM='+encodeURIComponent(TILDAPAGE_UTM)+'; path=/; expires='+date.toUTCString()}}}}catch(err){}};$(document).ready(function(){var myNav=navigator.userAgent.toLowerCase();var isIE=myNav.indexOf('msie')!=-1?parseInt(myNav.split('msie')[1]):!1;if(isIE==8||isIE==9){$('.t-btn').each(function(){var url=$(this).attr('href');if($(this).find('table').length>0&&url>''&&url.indexOf('#popup:')==-1&&url.indexOf('#price:')==-1){$(this).click(function(e){e.preventDefault();var url=$(this).attr('href');window.location.href=url})}})}
